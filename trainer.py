...
class Trainer:
    def __init__(self, dataset, args,autoencoder):
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        self.dataset = dataset
        self.args = args
        self.autoencoder = autoencoder
        self.model = SimplE(dataset.num_ent(), dataset.num_rel(), self.autoencoder, args.emb_dim, self.device)
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.ne + 1):
            ...
            while not last_batch:
                ...
                optimizer.zero_grad()
                scores,recon_error = self.model(h, r, t)
                loss = torch.sum(F.softplus(-l * scores)) + recon_error + (self.args.reg_lambda * self.model.l2_loss() / self.dataset.num_batch(self.args.batch_size))
                loss.backward()
                optimizer.step()
